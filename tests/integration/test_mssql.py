...
def setup_env():
    ...
    source_env = Path("tests/integration/inputs/mssql_sqlconnect.env")
...
def setup_connections():
    ...
    source_env = Path("tests/integration/inputs/mssql_sqlconnect.yaml")
...
def test_sql_to_df_str_postgres(setup_env, setup_connections):
    conn = sc.Sqlconnector("Mssql")

    df = conn.sql_to_df_str(
        "SELECT name FROM dbo.employees WHERE position = 'Data Engineer'"
    )
